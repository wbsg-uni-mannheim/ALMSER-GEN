...
def createVectorsFromFiles(path, id_attr):
    
    #create dir for saving the feature vectors
    if not os.path.exists(path+'feature_vector_files/'):
        os.makedirs(path+'feature_vector_files/')
    
    matching_tasks = list([os.listdir(path+"blocked_pairs/")][0])
    for task in matching_tasks:
        task_name = task.replace('.csv','')
        source_name = task_name.split('_')[0]
        target_name = task_name.split('_')[1]
        
        source_data = pd.read_csv(path+"sources/"+source_name+".csv")
        source_data[id_attr] = source_data[id_attr].astype(str)
        target_data = pd.read_csv(path+"sources/"+target_name+".csv")
        target_data[id_attr] = target_data[id_attr].astype(str)

        blocked_pairs=  pd.read_csv(path+"blocked_pairs/"+task)
        blocked_pairs['source_id'] = blocked_pairs['source_id'].astype(str)
        blocked_pairs['target_id'] = blocked_pairs['target_id'].astype(str)

        createFeatureVectorFile(source_data,target_data,blocked_pairs,keyfeature=id_attr, embeddings = False, predefinedTypes = dict(), printProgress=True, saveFile=path+'feature_vector_files/'+task, tfidf_grams=1, threads=1)
...
def createFeatureVectorFile(source,target,pool, keyfeature='id', embeddings = True, predefinedTypes = dict(), printProgress=False, saveFile=False, tfidf_grams=1, threads=10):    
    try:
        ...
        features_df['source_id'] = copy.copy(pool['source_id'])
        features_df['target_id'] = copy.copy(pool['target_id'])
        features_df['pair_id'] = features_df['source_id'].astype(str) + '-'+features_df['target_id'].astype(str)

        features_df['label'] = copy.copy(pool['matching'])
        ...
        def featureVectorPerPair(pair_index):
            try:
                
                r_source_id = features_df.iloc[pair_index]['source_id']
                r_target_id = features_df.iloc[pair_index]['target_id']
                
                if (r_source_id==-1 or r_source_id=='-1'):
                    print("WRONG POINT 1")
                    import pdb;pdb.set_trace();
                ...
                for f in common_elements:
                    ...
                    fvalue_source= str(source[source[keyfeature] == r_source_id][f].values[0])
                    
                    fvalue_target = str(target[target[keyfeature] == r_target_id][f].values[0])
                ...
                print("Problem with pair ", pair_index)
                print(pool['source_id'].iloc[pair_index])
                print(pool['target_id'].iloc[pair_index])
                import pdb;pdb.set_trace();
        
        #parallel execution
        Parallel(n_jobs=threads, prefer="threads")(delayed(featureVectorPerPair)(i) for i in tqdm(features_df.index.values))  
        #now rescale to 0-1        
        if (-1 in features_df.source_id.values or '-1' in features_df.source_id.values):
            print("WRONG POINT 2")
            import pdb;pdb.set_trace();
