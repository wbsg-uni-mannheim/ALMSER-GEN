...
def getNestedXValidationResults(data):
...
def getHeatmapOfSetting(fv, distinct_sources, sort_tasks_in_heatmap=list()):
    
    print("Calculate and display heatmap of the complete matching setting")
    if len(sort_tasks_in_heatmap)>0:
        results_hmap = pd.DataFrame(index=sort_tasks_in_heatmap, columns=sort_tasks_in_heatmap)
    else:       
